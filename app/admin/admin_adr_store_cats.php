...
if ( $mode != "" )
{
	switch( $mode )
	{
		...
		case 'delete_store':
			...
			$store_id = ( !empty($_POST['store_id']) ) ? intval($_POST['store_id']) : intval($_GET['store_id']);
		...
		case "save_store":
			...
			$store_name = ( isset($_POST['item_name']) ) ? trim($_POST['item_name']) : trim($_GET['item_name']);
			$store_desc = ( isset($_POST['item_desc']) ) ? trim($_POST['item_desc']) : trim($_GET['item_desc']);
			$store_img = ( isset($_POST['item_img']) ) ? trim($_POST['item_img']) : trim($_GET['item_img']);
//			$store_status = intval($_POST['item_status']);
//			$store_sales = intval($_POST['item_sales']);
			$store_status = intval(1);
			$store_sales = intval(0);
