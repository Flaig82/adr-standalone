...
else if ( $mode != "" )
{
	switch( $mode )
	{
		case 'delete':
			...
			$stock_id = ( !empty($_POST['id']) ) ? intval($_POST['id']) : intval($_GET['id']);
		...
		case "save":
			...
			$stock_name = ( isset($_POST['stock_name']) ) ? trim($_POST['stock_name']) : trim($_GET['stock_name']);
			$stock_desc = ( isset($_POST['stock_desc']) ) ? trim($_POST['stock_desc']) : trim($_GET['stock_desc']);
			$stock_price = ( isset($_POST['stock_price']) ) ? intval($_POST['stock_price']) : intval($_GET['stock_price']);
			$stock_id = ( isset($_POST['stock_id']) ) ? intval($_POST['stock_id']) : intval($_GET['stock_id']);
