...
function adr_use_skill_mining($user_id , $tool)
{
	...
	if ( $user_chance > $rand )
	{
		...
		$item_name = ( $new_item_type == 1 ) ? 'Adr_item_ore' : 'Adr_items_diamond';
		$item_desc = ( $new_item_type == 1 ) ? 'Adr_item_ore_desc' : 'Adr_items_diamond_desc';
		$item_icon = ( $new_item_type == 1 ) ? 'ore.gif' : 'diamond.gif' ;
		...
		$sql = "INSERT INTO " . ADR_SHOPS_ITEMS_TABLE . " 
			...
			VALUES ( $new_item_id , $user_id , $item_type , '" . str_replace("\'", "''", $item_name) . "', '" . str_replace("\'", "''", $item_desc) . "' , '" . str_replace("\'", "''", $item_icon) . "' , $item_price , $item_quality , $item_duration, $item_duration , $item_power )";
		$result = $db->sql_query($sql);
		if( !$result )
		{
