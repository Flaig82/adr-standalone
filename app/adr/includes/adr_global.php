...
@include_once($phpbb_root_path . 'adr/language/lang_' . $board_config['default_lang'] . '/lang_adr_faq.' . $phpEx);
...
// Standalone: skip the adr_db_update.php check
// if(file_exists($phpbb_root_path . 'adr_db_update.'.$phpEx)){
// 	$message = $lang['Adr_disabled_dbupdate_error'];
...
function adr_template_file($file)
{
	...
	// Standalone: always use adr/templates/ path
	$template->set_filenames(array(
		"body" => "adr/templates/$file")
	);
}
...
function adr_make_bars($hp, $hp_max, $width_value)
{
	...
	{
		$hp_percent_width = ($hp_max > '0')? floor(($hp /$hp_max) *$width_value) : intval(0);
