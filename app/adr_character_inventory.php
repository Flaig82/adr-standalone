...
if ( $mode != "" )
{
	...
	switch($mode)
	{
		...
		case 'give' :
			...
			$sql = "SELECT u.user_id, u.username, c.character_name, c.character_id FROM " . USERS_TABLE . " u, " . ADR_CHARACTERS_TABLE . " c
			...
			for($t = 0; $t < count($users); $t++){
				$give_to .= '<option value = "'.$users[$t]['character_id'].'">' . $users[$t]['character_name'] . '&nbsp;('.$users[$t]['username'].')&nbsp;</option>';}
			$give_to .= '</select>';
		...
		case "save_item":
			...
			$item_price = intval($_POST['item_price']);
				...
				$sql = "UPDATE " . ADR_SHOPS_ITEMS_TABLE . "
					...
					WHERE item_id = '" . str_replace("\'", "''", $item_id) . "'
					AND item_owner_id = $user_id ";
				if( !($result = $db->sql_query($sql)) )
				{
					message_die(GENERAL_ERROR, $lang['Adr_shop_items_failure_deleted']);
