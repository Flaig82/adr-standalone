...
if ( $mode != "" )
{
	switch($mode)
	{
		...
		case 'accept_action' :
			...
			if ( $armor )
			{
				$sql = " SELECT item_power, item_add_power FROM " . ADR_SHOPS_ITEMS_TABLE . "
					WHERE item_in_shop = 0 
					AND item_owner_id = $user_id
					$item_sql
					AND item_id = $armor ";
			...
			if ( $amulet )
			{
				$sql = " SELECT item_power FROM " . ADR_SHOPS_ITEMS_TABLE . "
					WHERE item_in_shop = 0 
					AND item_owner_id = $user_id
					$item_sql
					AND item_id = $amulet ";
...
//       header('Location: ' . append_sid("adr_battle_pvp.$phpEx?battle_id=$battle_id", true));
		...
		case 'defy_action' :
			...
			$defied = (!empty($_POST['defied'])) ? intval($_POST['defied']) : intval($_GET['defied']);
