...
$celled_list = $_POST['celled_list'];
$caution = $_POST['caution_pay'];
$guilty = $_POST['guilty'];
$innocent = $_POST['innocent'];
$blank = $_POST['blank'];
$celled_id = intval($_POST['celled_id']);
$celled_user_id = intval($_GET['celled_user_id']);

if ( isset($_GET['from']) || isset($_POST['from']) )
{
	$from = ( isset($_POST['from']) ) ? htmlspecialchars($_POST['from']) : htmlspecialchars($_GET['from']);
}
else
{
	$from = 'list';
...
switch( $from )
{
	...
	case 'caution_pay' :
		...
		$sledge_price = intval($_POST['sledge_price']);
